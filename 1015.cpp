...
int main() {
	...
	cnt = n;
	for (int i = 0; i < n; i++) {
		cin >> student.num >> student.score_de >> student.score_cai;
		if (student.score_de < l || student.score_cai < l)
			cnt--;
		else if (student.score_de >= h && student.score_cai >= h)
			stu[0].push_back(student);
		else if (student.score_de >= h && student.score_cai >= l)
			stu[1].push_back(student);
		else if (student.score_de >= l && student.score_cai >= l && student.score_de >= student.score_cai)
			stu[2].push_back(student);
		else
			stu[3].push_back(student);
	}
	cout << cnt << endl;
	for (int i = 0; i < 4; i++) {
		sort(stu[i].begin(), stu[i].end(), cmp);
		for (auto it : stu[i])
			cout << it.num << " " << it.score_de << " " << it.score_cai << endl;
			//printf("%d %d %d\n", it.num, it.score_de, it.score_cai);
	}
	return 0;
}